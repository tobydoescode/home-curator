...
async def test_fake_records_update_calls():
    ...
    await fake.update_device("d1", HADeviceUpdate(area_id="kitchen"))
    assert fake.update_calls == [("d1", HADeviceUpdate(area_id="kitchen"))]
...
async def test_fake_emits_subscribed_events():
    ...
    await fake.emit(DeviceUpdatedEvent(device_id="d1"))
    assert received == [DeviceUpdatedEvent(device_id="d1")]
...
async def test_unsubscribe_stops_delivery_and_is_idempotent():
    ...
    await fake.emit(AreaUpdatedEvent())
...
async def test_fake_delete_device_removes_from_store_and_records_call():
    ...
    ha = FakeHAClient(
        devices=[
            HADevice(id="d1", name="a", config_entries=["e1"]),
            HADevice(id="d2", name="b", config_entries=["e2"]),
        ],
        areas=[],
    )
...
async def test_fake_records_update_entity_calls_and_mutates_store():
    ...
    await fake.update_entity("light.lamp", HAEntityUpdate(name="Kitchen Lamp"))
    assert fake.update_entity_calls == [("light.lamp", HAEntityUpdate(name="Kitchen Lamp"))]
    ents = await fake.get_entities()
...
async def test_fake_delete_entity_removes_from_store_and_records_call():
    fake = FakeHAClient(
        ...
        areas=[],
        entities=[
            HAEntity(entity_id="light.a", platform="hue", device_id=None),
            HAEntity(entity_id="light.b", platform="hue", device_id=None),
        ],
    )
...
async def test_fake_emits_entity_events_to_subscribers():
    ...
    received = []
    fake.subscribe(lambda event: received.append(event))
    await fake.emit(EntityUpdatedEvent(entity_id="light.lamp"))
    await fake.emit(EntityDeletedEvent(entity_id="light.lamp"))
    assert received == [
        EntityUpdatedEvent(entity_id="light.lamp"),
        EntityDeletedEvent(entity_id="light.lamp"),
    ]
